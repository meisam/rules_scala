...
convert_msys2_path() {
  ...
  # Bazel stores paths using Unix style forward slashes, so it interprets
  # Windows path strings that use backslashes as relative:
  #
  # - https://bazel.build/rules/windows#paths
  #
  # This is why we invoke `cygpath -m`, which emits forward slashes, instead of
  # `cygpath -w`.
